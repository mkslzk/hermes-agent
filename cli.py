...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def _show_billing(self, command: str = "/billing"):
        """`/billing [buy|auto-reload|limit]` — terminal billing for Nous.

        Screens (plan §"The 5 screens"):
          (no arg)      → Overview (Screen 1) + interactive menu
          buy           → Buy credits (Screen 3) → Confirm + charge + poll (Screen 4)
          auto-reload   → Auto-reload config (Screen 2)
          limit         → Monthly spend limit (Screen 5, read-only)
        ...
        if not state.logged_in:
            print()
            if state.error:
                print(f"  💳 {_DIM}Couldn't load billing: {state.error}{_RST}")
            else:
                print(f"  💳 {_DIM}Not logged into Nous Portal.{_RST}")
                print("  Run `hermes portal` to log in, then /billing.")
            return

        if sub in ("buy", "credits", "topup", "top-up"):
            self._billing_buy_flow(state)
        elif sub in ("auto-reload", "autoreload", "auto", "reload"):
            self._billing_auto_reload_flow(state)
        elif sub in ("limit", "cap", "monthly"):
    ...
    def _billing_overview(self, state):
        ...
        print()
        print(f"  💳 {_BOLD}Usage credits{_RST}")
        print(f"  {'─' * 41}")
        ...
        if cap is not None and cap.limit_usd is not None:
            ...
            print(f"  This month: {spent} of {limit} used{ceiling}")
        ...
        if state.org_name:
            ...
            print(f"  {_DIM}Org: {state.org_name}{f' · {role}' if role else ''}{_RST}")
        print(f"  {'─' * 41}")

        # Action gating: admin + kill-switch for charge/auto-reload; everyone gets portal.
        if not state.is_admin:
            print(f"  {_DIM}Billing actions require an org admin/owner.{_RST}")
            self._billing_portal_hint(state)
            return
        if not state.cli_billing_enabled:
            print(f"  {_DIM}Terminal billing is turned off for this org.{_RST}")
            self._billing_portal_hint(state, reason="Enable it on the portal to buy credits here.")
            return

        # Non-interactive: list the subcommands as text (no modal).
        if not getattr(self, "_app", None):
            print("  Actions: /billing buy · /billing auto-reload · /billing limit")
            self._billing_portal_hint(state)
            return
        ...
        choices = [
            ...
            ("limit", "View monthly limit", "show the monthly spend cap (read-only)"),
            ("portal", "Manage on portal", "open the billing page in your browser"),
            ("cancel", "Cancel", "do nothing"),
        ]
        raw = self._prompt_text_input_modal(
            title="💳 Usage credits", detail=f"Balance: {format_money(state.balance_usd)}",
            choices=choices,
        )
        choice = self._normalize_slash_confirm_choice(raw, choices)
        if choice == "buy":
    ...
    def _billing_buy_flow(self, state):
        """Screen 3 (select) → Screen 4 (confirm + charge + poll)."""
        ...
        if not getattr(self, "_app", None):
            ...
            print()
            print(f"  💳 {_BOLD}Buy usage credits{_RST}")
    ...
    def _billing_confirm_and_charge(self, state, amount):
        """Screen 4 — confirm totals + consent, charge, then poll to settlement."""
        from decimal import Decimal
        ...
        card = state.card
        print()
        print(f"  💳 {_BOLD}Confirm purchase{_RST}")
        print(f"  {'─' * 41}")
        print(f"  Subtotal:  {format_money(amount)}")
        print(f"  Tax (0%):  {format_money(Decimal('0'))}")
        print(f"  Total due: {format_money(amount)}")
        if card:
            print(f"  Payment:   {card.masked}")
        print(f"  {'─' * 41}")
        print(f"  {_DIM}By confirming, you allow Nous Research to charge your card "
              f"in the amount above.{_RST}")
        ...
        if not charge_id:
            ...
            return
        print(f"  {_DIM}Charge submitted — confirming settlement…{_RST}")
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if code == "no_payment_method":
            ...
            print("  🔴 Terminal billing is turned off for this org. Enable it on the portal.")
    ...
    def _billing_handle_scope_required(self, state):
        ...
        print(f"  {_DIM}An org admin/owner must tick \"Allow terminal billing\" during "
              f"login.{_RST}")
        if not getattr(self, "_app", None):
        ...
        try:
            ...
            return
        if granted:
            print("  ✅ Terminal billing enabled. Run /billing buy again to continue.")
        else:
            print("  🟡 Terminal billing was not granted (an admin must tick the box).")

    def _billing_auto_reload_flow(self, state):
        """Screen 2 — auto-reload config: threshold + reload-to → PATCH."""
        from agent.billing_view import format_money, validate_charge_amount
        ...
        card = state.card
        print()
        print(f"  💳 {_BOLD}Auto-reload{_RST}")
        print(f"  {'─' * 41}")
        if card:
        ...
        if ar and ar.enabled:
            print(f"  Currently: below {format_money(ar.threshold_usd)} → "
                  f"reload to {format_money(ar.reload_to_usd)}")
        ...
        threshold_raw = self._prompt_text_input("  Reload when balance falls below (USD): ")
        tv = validate_charge_amount(threshold_raw or "", min_usd=None, max_usd=None)
        if not tv.ok or tv.amount is None:
            print(f"  🔴 {tv.error}")
            return
        threshold_amt = tv.amount
        reload_raw = self._prompt_text_input("  Reload balance to (USD): ")
        rv = validate_charge_amount(reload_raw or "", min_usd=None, max_usd=None)
        if not rv.ok or rv.amount is None:
            print(f"  🔴 {rv.error}")
            return
        reload_amt = rv.amount
        if reload_amt <= threshold_amt:
            print("  🔴 Reload-to amount must be greater than the threshold.")
            return

        print()
        print(f"  {_DIM}By turning this on, you authorize Nous Research to "
              f"automatically charge {card.masked} whenever your balance falls "
              f"below {format_money(threshold_amt)}. Turn off any time here or on the "
              f"portal.{_RST}")
        confirm_choices = [
    ...
    def _billing_limit_screen(self, state):
        ...
        print()
        print(f"  💳 {_BOLD}Monthly spend limit{_RST}")
        print(f"  {'─' * 41}")
        cap = state.monthly_cap
        if cap is None or cap.limit_usd is None:
            print(f"  {_DIM}No monthly cap visible (managed on the portal).{_RST}")
        else:
            spent = format_money(cap.spent_this_month_usd)
            limit = format_money(cap.limit_usd)
            ceiling = " (default ceiling)" if cap.is_default_ceiling else ""
            print(f"  {spent} of {limit} used this month{ceiling}")
        print(f"  {_DIM}The monthly limit is set on the portal — the terminal shows "
              f"it read-only.{_RST}")
        self._billing_portal_hint(state)
